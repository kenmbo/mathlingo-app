...
const QuizApp = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        //const response = await window.fs.readFile('output_example.json', { encoding: 'utf8' });
        // Parse the JSON twice since it's a string containing a JSON string
        //const jsonString = JSON.parse(response);
        //const data = JSON.parse(jsonString);

	const response = await fetch('/data/quiz_data.json');
	const jsonString = await response.json();
	const data = jsonString; // No need for double parsing with fetch
